...
export default memo(function InfoNode({ data }: NodeProps) {
  ...
  return (
    <div className={`w-[250px] px-4 py-3 rounded-lg border-2 bg-card shadow-sm border-yellow-500 transition-all ${
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <div className="font-medium text-sm flex-1">Info Message</div>
        <button
      ...
      {!isCollapsed && (
        <>
          ...
          {nodeData.description && (
            <div className="text-xs text-muted-foreground break-words whitespace-normal">{nodeData.description}</div>
          )}
        </>
      )}
