...
import { UserPlus, Trash2, Mail, Shield, Briefcase, Users, Key } from "lucide-react";
...
export function TeamTabContent() {
  ...
  const [confirmReset, setConfirmReset] = useState<{userId: string; userName: string} | null>(null);
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setInvitedEmail(inviteForm.email); // Save the email before clearing form
  ...
  const confirmRemoveAction = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/team?userId=${confirmRemove.userId}`, {
        method: "DELETE",
      });
  ...
  const handleResetPassword = async (userId: string, userName: string) => {
    setConfirmReset({ userId, userName });
  };
  ...
  const confirmResetAction = async () => {
    ...
    try {
      const response = await fetch("/api/admin/team/reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: confirmReset.userId }),
      });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Invite paralegals and staff to help manage your practice
          </p>
        </div>
        <Button
      ...
      {/* Team Members Table */}
      {loading ? (
        <p className="text-gray-600 py-8 text-center">Loading team members...</p>
                    ...
                    {member.role !== "org_admin" && (
                      <div className="flex gap-2 justify-end">
                        <Button
                          ...
                          title="Reset password to 12345678 (simulates email reset)"
                        >
              ...
              <div className="space-y-4">
                <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <p className="text-sm text-green-800 mb-3">
                    Send these credentials to the new team member:
                  </p>
                  ...
                  <p className="text-xs text-green-700 mt-2">
                    💡 The user can log in with this password. You can also reset it to "12345678" using the "Reset Password" button.
                  </p>
                </div>
                <Button
                  onClick={() => {
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                      <input
                        ...
                        onChange={() => setInviteForm({ ...inviteForm, role: "staff" })}
                      />
                      <div>
                    ...
                    <label className="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                      <input
                        ...
                        onChange={() => setInviteForm({ ...inviteForm, role: "attorney" })}
                      />
                      <div>
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to remove ${confirmRemove?.userName} from your team? This action cannot be undone.`}
        confirmText="Remove"
        cancelText="Cancel"
        variant="destructive"
      ...
      <ConfirmDialog
        ...
        description={`Reset password for ${confirmReset?.userName} to "12345678"? This simulates sending a password reset email. The user can log in with this new password.`}
